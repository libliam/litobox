...
export const changelogData: ChangelogData = {
  ...
  },
  {
    "version": "8.1.0",
    "date": "2026-08-31",
    "content": "编码转换新增人民币大写转换：输入数字金额输出中文大写（如 123456.78 → 壹拾贰万叁仟肆佰伍拾陆元柒角捌分），支持万亿级、角分与零的正确处理，非法输入红色报错；同时支持多行批量转换：操作区新增\"多行批量\"开关，开启后 URL/Base64/HTML实体/Unicode 编解码、时间戳双向转换、人民币大写均按行逐条转换（自动去空行），输出与输入逐行对应，单行无效时标记\"无效: 原行\"不中断整体",
    "isEnhancement": false,
    "isNewFeature": true
  },
  {
    "version": "8.0.3",
    "date": "2026-08-31",
    "content": "随机数据工具优化：25 张卡片按业务语义重排（个人信息→网络账号→企业财务→物流交通→地理/标识→内容）；每张卡片新增批量复制按钮；顶部新增统一数量输入，全部生成时所有类型按该数量一键生成（不修改各卡片独立配置），完成后仅提示一次；金额卡片新增人民币大写开关，切换开关时已生成结果实时在大写/数字间重算（无需重新生成），参数区改为两行布局避免挤压",
    "isEnhancement": true,
    "isNewFeature": true
  },
  {
