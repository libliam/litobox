...
| F13 | **视频裁剪工具**                | 起止裁剪/片段提取。先做 MP4 关键帧无损裁剪（mp4 crate，+1-2 MB，覆盖 \~90% 日常视频）；ffmpeg 作为可选外部依赖运行时探测，不内嵌打包。需帧精确/全格式时降级提示                      | 中            | 2026-07-08 brainstorming |
...
| F24 | **视频截图/帧提取**     | 抽取指定时间点帧为图片，或按间隔批量截图（如每 10 秒一帧），导出 PNG/JPG。场景：视频封面制作、内容预览。仅 ffmpeg 模式可用                                                                     | 高            | 2026-07-17 视频工具扩展        |
| F25 | **视频画面裁剪（区域）**     | 画面空间裁剪，去黑边/裁出指定区域，与时间裁剪 F13 互补。支持手动输入宽高偏移或预设比例（16:9/4:3/1:1）。仅 ffmpeg 模式可用                                                                     | 高            | 2026-07-17 视频工具扩展        |
...
| #   | 方向   | 说明                                  | 状态     |
| --- | ---- | ----------------------------------- | ------ |
| F20 | 音频格式转换 | MP3/WAV/M4A/FLAC/OGG 互转，仅 ffmpeg | ✅ 已完成 V5.4 |
| F21 | 音频压缩 | 目标比特率/质量等级，仅 ffmpeg | ✅ 已完成 V5.4 |
| F22 | 音频合并 | 多文件拼接+拖拽排序，仅 ffmpeg | ✅ 已完成 V5.4 |
| F23 | 变速变调 | 0.5x~4.0x，保持音调可选，仅 ffmpeg | ✅ 已完成 V5.4 |
...
- [ ] F24 视频截图/帧提取 或 F25 视频画面裁剪（ffmpeg 已接入，实现成本低）
