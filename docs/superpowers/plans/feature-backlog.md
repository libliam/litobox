...
| F12 | **音频裁剪工具**                | 起止裁剪/片段提取/多段拼接/淡入淡出。纯 Rust（symphonia+hound+lame-sys），无损帧切+精确切两档，新增体积约 3-5 MB                                            | 中            | 2026-07-08 brainstorming |
...
| F16 | **视频转码**                  | 视频格式互转（MP4/MKV/AVI/MOV/WebM），支持选择编码器（H264/H265/VP9）、分辨率、帧率、比特率。仅 ffmpeg 模式可用                                            | 高            | 2026-07-16 视频工具扩展        |
| F17 | **音频提取**                  | 从视频中提取音频，支持导出 MP3/AAC/WAV/FLAC/OGG 格式，可选音频质量/比特率。仅 ffmpeg 模式可用                                                          | 高            | 2026-07-16 视频工具扩展        |
| F18 | **视频压缩**                  | 视频体积压缩，可调 CRF/比特率/分辨率/编码器，实时预估压缩比，保留原始分辨率选项。仅 ffmpeg 模式可用                                                               | 高            | 2026-07-16 视频工具扩展        |
| F19 | **视频合并**                  | 多个视频文件拼接为一个，支持同格式无损合并（concat demuxer）和不同格式转码合并（concat filter），可调输出格式和编码器。仅 ffmpeg 模式可用                                  | 高            | 2026-07-16 视频工具扩展        |
...
| #   | 方向   | 说明                                  | 状态     |
| --- | ---- | ----------------------------------- | ------ |
| F16 | 视频转码 | 格式互转 + 编码器/分辨率/帧率/比特率可调，仅 ffmpeg    | 🚧 开发中 |
| F17 | 音频提取 | 视频→音频，MP3/AAC/WAV/FLAC/OGG，仅 ffmpeg | 🚧 开发中 |
| F18 | 视频压缩 | CRF/比特率/分辨率/编码器可调，仅 ffmpeg          | 🚧 开发中 |
| F19 | 视频合并 | 多文件拼接，同格式无损/跨格式转码合并，仅 ffmpeg        | 🚧 开发中 |
