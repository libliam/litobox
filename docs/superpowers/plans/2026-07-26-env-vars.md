# 环境变量编辑器 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [x]`) syntax for tracking.

**Goal:** 新增独立的环境变量管理工具，支持查看/新增/修改/删除 Windows 用户级和系统级环境变量，PATH 变量逐行编辑。

**Architecture:** 新增 `env_vars.rs` 后端模块（PowerShell 读写注册表 + WM_SETTINGCHANGE 广播），新增 `EnvVarsView.vue` 前端页面（双 Tab + 表格行内编辑 + PATH 逐行编辑器），参照 `startup_items` 的完整实现模式。

**Tech Stack:** Rust (PowerShell 子进程, serde_json), Vue 3 + Element Plus + TypeScript

---

### Task 1: 创建后端 env_vars.rs 模块

**Files:**
- Create: `src-tauri/src/env_vars.rs`

- [x] **Step 1: 创建完整的 env_vars.rs 文件** ✅
...
- [x] **Step 2: 运行 Rust 测试验证**

```bash
cd src-tauri && cargo test --lib env_vars
```
...
```bash
git add src-tauri/src/env_vars.rs
git commit -m "feat: 新增环境变量管理后端模块"
```

---

### Task 2: 在 main.rs 中注册模块和命令
...
在 `src-tauri/src/main.rs` 第 25 行 `mod startup_items;` 之后添加：

```rust
mod env_vars;
```

- [x] **Step 2: 注册 Tauri 命令**

在 `src-tauri/src/main.rs` 的 `startup_items::add_startup_item,` 之后添加：
...
git commit -m "feat: 注册环境变量管理模块和命令"
```

---

### Task 3: 添加前端类型和 API 封装

**Files:**
- Modify: `src/utils/systemInfoClient.ts`

- [x] **Step 1: 在 systemInfoClient.ts 末尾添加类型和函数**
...
git commit -m "feat: 添加环境变量管理前端类型和 API 封装"
```

---

### Task 4: 创建前端页面 EnvVarsView.vue

**Files:**
- Create: `src/views/EnvVarsView.vue`

- [x] **Step 1: 创建完整的 EnvVarsView.vue**
            ...
            <template #default="{ row }">
              ...
              <template v-if="row.name === 'Path' || row.name === 'PATH'">
                <div v-if="pathExpanded === row.rowKey" class="path-editor">
                  <div v-for="(p, pi) in pathEntries" :key="pi" class="path-row">
...
import {
  ...
  type EnvVarInfo, type EnvVarResult,
...
```bash
git add src/views/EnvVarsView.vue
git commit -m "feat: 新增环境变量编辑器前端页面"
```
...
- [x] **Step 1: 在 TOOL_LIST 中添加条目**

在 `src/store/index.ts` 第 100 行 `startupItems` 条目之后添加：
...
- [x] **Step 2: 在 App.vue 中注册路由**

在 `src/App.vue` 第 88 行 `import StartupItemsView` 之后添加：

```typescript
import EnvVarsView from '@/views/EnvVarsView.vue'
```

在 `src/App.vue` 第 149 行 `startupItems: StartupItemsView,` 之后添加：
...
git commit -m "feat: 注册环境变量编辑器路由和菜单"
```
...
- [x] **Step 1: 更新版本号 6.3.0 → 6.4.0**

在 `package.json` 第 3 行：
```
"version": "6.3.0",
```
改为：
...
- [x] **Step 1: 在 README.md 版本路线中添加 V6.4**

在 `src/views/StartupItemsView.vue` 的 README 版本路线中，V6.3 条目之后添加：

```markdown
| V6.4 | ✅ | 环境变量编辑器：查看/新增/修改/删除用户级和系统级环境变量，PATH 逐行列表编辑，表格行内编辑，CSV 导出，操作历史记录 | 2026-07-26 |
```

- [x] **Step 2: 更新 feature-backlog.md**

将 A6 条目从候选池移到已完成，标记为已完成 V6.4：
...
改为：
```
| A6  | ✅ **环境变量编辑**       | 在软件环境的 env var 基础上，支持新增/修改/删除用户级环境变量 — 已完成 V6.4                                                                                                                                       | — 已完成 V6.4 — | 系统工具扩展                   |
```

在已完成版本表中添加：
```
| V6.4 | ✅  | 环境变量编辑器（查看/新增/修改/删除用户级和系统级环境变量，PATH 逐行编辑，表格行内编辑，CSV导出）                                 | 2026-07-26 |
```

- [x] **Step 3: 提交**

```bash
git add README.md docs/superpowers/plans/feature-backlog.md
git commit -m "docs: 更新文档至 V6.4 环境变量编辑器"
```
